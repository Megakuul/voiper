...
<center style="background-color: {Palette.bgPrimary()};" class="h-full box-border">
  <div class="flex flex-row justify-around gap-4 m-6 max-w-[1200px]">
    <div title="Linus | 10.1.10.252" 
      style="color: {Palette.fgSecondary()}; border-color: {Palette.fgPrimary()};" 
      class="w-16 hover:w-52 rounded-2xl border-2 transition-all duration-700 overflow-hidden">
      <div class="w-52 flex flex-row items-center gap-6 py-3 px-5">
        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"><circle cx="5" cy="19" r="0" fill="currentColor"><animate fill="freeze" attributeName="r" dur="0.2s" values="0;2"/></circle><g fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2"><path stroke-dasharray="16" stroke-dashoffset="16" d="M4 11c2.39 0 4.68 0.95 6.36 2.64c1.69 1.68 2.64 3.97 2.64 6.36"><animate fill="freeze" attributeName="stroke-dashoffset" begin="0.2s" dur="0.3s" values="16;0"/></path><path stroke-dasharray="28" stroke-dashoffset="28" d="M4 4c4.24 0 8.31 1.69 11.31 4.69c3 3 4.69 7.07 4.69 11.31"><animate fill="freeze" attributeName="stroke-dashoffset" begin="0.6s" dur="0.4s" values="28;0"/></path></g></svg>
        <p class="flex-1/2 flex flex-col ml-auto overflow-hidden">
          <span class="font-bold text-sm text-start text-nowrap">{activeState}</span>
          <!--
          <span title="{activeState}" 
            class="text-xs text-start text-nowrap">
            {activeConfig.DisplayName} | {activeConfig.Server}
          </span>
          -->
        </p>
      </div>
    ...
    <button id="home-nav"
      style="color: {Palette.fgSecondary()}; border-color: {Palette.fgPrimary()};" 
      class="flex flex-row items-center gap-2 hover:bg-slate-600/10 py-3 px-5 md:px-20 rounded-2xl border-2 transition-all duration-700"
      onclick={() => Page = "home"}>
  ...
  {:else if Page=="logs"}
    <Logs bind:LogEntriesRef={LogEntries} bind:ExceptionRef={Exception}></Logs>
  {/if}
</center>
